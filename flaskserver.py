...
from parse_qdrant_output import generate_signed_url
...
def upload_file():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            embeddings_from_upload = do_inference("img2vec50", os.path.join(app.config["UPLOAD_FOLDER"], filename))
            # Query QDRANT
            num_qdrant_results = int(request.form["number_of_results"])
            requested_image_type = "".join([key for key, value in json.loads(request.form["image_type"]).items() if value])
            assert len(requested_image_type) == 1
            requested_diagnoses = [key.replace("Gliomas","glioma").replace("Meningiomas","men").replace("Metastasis","met") # change because disgnoses are stored differnetly on QDRANT
                                    for key, value in  json.loads(request.form["cancer_types"]).items() 
                                    if value
                                    ]
            results = {diagnose:generate_signed_url(
